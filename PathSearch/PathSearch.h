...
#define USEDEFAULTMAP hex006x006
...
namespace fullsail_ai { namespace algorithms {
	...
	private:
		...
		PriorityQueue<PlannerNode*, GreedyComparitor> openQueue;
	...
	public:
		...
		DLLEXPORT void GetEvenNeigbors(int row, int col, SearchNode* CurrNode);

		DLLEXPORT void GetOddNeighbors(int row, int col, SearchNode* CurrNode);

		DLLEXPORT float estimate(SearchNode* start, SearchNode* goal);
	};
